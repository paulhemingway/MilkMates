var express = require('express');
var mysql = require('mysql2');
var router = express.Router();

var connection = mysql.createConnection({
    host     : 'ec2-54-159-200-221.compute-1.amazonaws.com',
    port     : '3306',
    user     : 'milkmates',
    password : 'MilkmatesPassword11!!',
    database : 'test'
});

connection.connect(function(err){
    
    if(!err) {
        console.log("Database is connected ... ");    
    } else {
        console.log("Error connecting database ... ");    
        console.log(err);
    }
});

router.get('/GetAllListingsByUser', async function(req, res, next)
{
    const user = req.body.username;
    
    //TODO: Make this list all listings by User
    let response = await QueryDatabase("SELECT * FROM test WHERE firstname = '" + user + "';");
    
    console.log(response);
    res.send(response).status(200).end();
});

router.get('/GetAllListingsByZIP', async function(req, res, next)
{
    const zipCode = req.body.zipCode;
    
    //TODO: Make this list all listings by Zip code
    let response = await QueryDatabase("SELECT * FROM test WHERE firstname = '" + zipCode + "';");
    
    console.log(response);
    res.send(response).status(200).end();
});

router.post('/CreateListing', async function(req, res, next)
{
    //TODO: Figure out if anything else is needed
    const batchID = req.body.batchID;
    
    //TODO: Make this create a listing
    let response = await QueryDatabase("SELECT * FROM test WHERE firstname = '" + batchID + "';");
    
    console.log(response);
    res.send(response).status(200).end();
});

router.put('/UpdateListing', async function(req, res, next)
{
    //TODO: Figure out if anything else is needed
    const listingID = req.body.listingID;
    
    //TODO: Make this edit a listing
    let response = await QueryDatabase("SELECT * FROM test WHERE firstname = '" + listingID + "';");
    
    console.log(response);
    res.send(response).status(200).end();
});

router.delete('/DeleteListing', async function(req, res, next)
{
    const listingID = req.body.listingID;
    
    //TODO: Make this delete a listing
    let response = await QueryDatabase("SELECT * FROM test WHERE firstname = '" + listingID + "';");
    
    console.log(response);
    res.send(response).status(200).end();
});

async function QueryDatabase(sqlQuery)
{
    return new Promise((resolve, reject) =>
    {
        connection.query(sqlQuery, function(err, results)
        {
            if (err) reject(err);
            console.log(results);
            resolve(results);
        });

    })
}

module.exports = router;